...
// --- COMPONENTE PRINCIPAL (COM O LAYOUT QUE VOCÊ GOSTA) ---
export function PortfolioClient({ projects, companies }: { projects: any[], companies: any[] }) {
  ...
  const filteredProjects = projects.filter(project => {
    // 1. Filtro por Projeto Específico
    if (filterProject !== "all" && project.id !== filterProject) return false

    // 2. Filtro de Tempo (Quarter, Mês, Semana)
    // Se não tiver filtro de tempo, mostra o projeto
    if (filterQuarter === "all" && filterMonth === "all" && filterWeek === "all") return true

    // Se tiver filtro de tempo, o projeto só aparece se tiver UMA fase que bata com o filtro
    const hasMatchingPhase = project.children?.some((f: any) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {filteredProjects.length > 0 ? filteredProjects.map((project: any) => {
            // Lógica de Ordenação Cronológica e Filtragem das Fases
            const phases = project.children
                ?.filter((phase: any) => {
                    // Aplica os mesmos filtros nas fases individuais para limpar a timeline
